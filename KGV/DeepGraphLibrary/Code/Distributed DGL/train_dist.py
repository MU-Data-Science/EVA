...
def load_subtensor(g, seeds, input_nodes, device, load_feat=True):
    ...
    """
    batch_inputs = g.ndata['feat'][input_nodes].float().to(device) if load_feat else None
...
class NeighborSampler(object):
    ...
    def sample_blocks(self, seeds):
        ...
        blocks[-1].dstdata['label'] = batch_labels
        return blocks
    ...
    def inference(self, g, x, batch_size, device):
        ...
        for l, layer in enumerate(self.layers):
            ...
            for blocks in tqdm.tqdm(dataloader):
                ...
                h = x[input_nodes].float().to(device)
                h_dst = h[:block.number_of_dst_nodes()]
                # print(f"h: {h}, block: {block}, h_dst: {h_dst}")
...
def compute_acc(pred, labels, steps=0):
    """
    Compute the accuracy of prediction given the labels.
    """
    labels = labels.long()
    pickle.dump({'pred': th.argmax(pred, dim=1), 'gold': labels}, open(f'/mydata/dgl/general/{steps}.pkl'), 'rb')
    return (th.argmax(pred, dim=1) == labels).float().sum() / len(pred)

def evaluate(model, g, inputs, labels, val_nid, test_nid, batch_size, device, steps=0):
    """
    ...
    # print(f"model: {model}, inputs: {inputs}, labels: {labels}, val_nid: {val_nid}, test_nid: {test_nid}, batch_size: {batch_size}, device: {device}")
    model.eval()
    with th.no_grad():
        pred = model.inference(g, inputs, batch_size, device)
    model.train()
    return compute_acc(pred[val_nid], labels[val_nid], steps=steps), compute_acc(pred[test_nid], labels[test_nid], steps=steps)
...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0 and epoch != 0:
            start = time.time()
            val_acc, test_acc = evaluate(model.module, g, g.ndata['feat'],
                                         g.ndata['label'], val_nid, test_nid, args.batch_size_eval, device, steps=epoch)
